...
/**
 * @brief Sturcture of pair of indices.
 */
struct Indices2D {
    uint32_t i;
    uint32_t j;
...
struct InputParameters {
    ...
    /**
     * Indicator if an initial test translation step is taken without GP (1) or
     * if GP is used right after initial rotations (0).
     */
    KeyValuePair<int> inittrans_nogp = {"inittrans_nogp", 0};
    ...
    void print()
    {
        ...
        log_man << num_iter_initrot.key << " : " << num_iter_initrot.value
                << "\n";
        log_man << inittrans_nogp.key << " : " << inittrans_nogp.value << "\n";
