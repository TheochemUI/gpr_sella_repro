    ...
    const gpr::Observation& middle_point, const double stop_criteria)
{
    ...
    // `max(abs(G_R))`
    // TODO(rg): Take an input parameter and pick either norm, max_force_on_atom etc.
                                              ...
                                              gpr::Index_t& num_es1)
{
    ...
    if (disp1D_nearest > fabs(log(ratio_at_limit))) {
        ++num_es1;
        res = true;
    }
...
{
    H5Easy::File file("gpr_optim_out.h5", H5Easy::File::ReadWrite);
    H5Easy::dump(file,
