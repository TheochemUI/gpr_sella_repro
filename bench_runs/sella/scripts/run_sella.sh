...
case "$IS_HPC" in
    true|True|TRUE|1|yes|Yes|YES)
        ...
        pixi run python "${CWD}/run_sella.py" "$INDEX" --spin "$SPIN" --scfthresh "${SCF_THRESH}" --datzip "${SCRATCHZIP}"
        popd
    ...
    false|False|FALSE|0|no|No|NO|"")
        ...
        cd "${FULL_RUNDIR}"
        pixi run python "${CWD}/run_sella.py" "$INDEX" --spin "$SPIN" --scfthresh "${SCF_THRESH}" --datzip "${DATZIP}"
        ;;
    *)
